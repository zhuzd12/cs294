...
def sample(env, 
    ...
    paths = []
    """ YOUR CODE HERE """
    for _ in tqdm.tqdm(num_paths):
        obs, acs, n_obs, rewards ,costs = [], [], [], [], []
        ob = env.reset()
        steps = 0
        while True:
            obs.append(ob)
            ...
            if steps >= horizon or done:
                break
        path = {"state": np.array(obs), "next_state": np.array(n_obs), "reward": np.array(rewards), "action": np.array(acs)}
        paths.append(path)
    return paths
...
def plot_comparison(env, dyn_model, horizon=100):
    """
    ...
    """
    """ YOUR CODE HERE """
    ground_obs, predict_obs= [], []
    ob = env.reset()
    ground_obs.append(ob)
    predict_obs.append(ob)
    steps = 0
    while True:
        action = env.action_space.sample()
        predict_ob = dyn_model.predict(ob, action)
        ...
        if done or steps >= horizon:
            break
    plt.plot(np.array(ground_obs.shape[0]), )
    plt.show()
    pass
