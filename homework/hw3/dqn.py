...
def learn(env,
    ...
    t_q_nn = q_func(obs_tp1_float, num_actions, scope="target_q_func", reuse=False)
    Q_samp = rew_t_ph + (1 - done_mask_ph) * gamma * tf.reduce_max(t_q_nn, axis=1)
    Q_s = tf.reduce_sum(q_nn * tf.one_hot(act_t_ph, num_actions), axis=1)
    total_error = tf.reduce_mean(tf.square(Q_s - Q_samp))
    q_func_vars = tf.get_collection(tf.GraphKeys.GLOBAL_VARIABLES, scope='q_func')
    ...
    for t in itertools.count():
        ...
        if np.random.random() < exploration.value(t) or not model_initialized:
            ...
            action_value = session.run(q_nn, feed_dict={obs_t_float:[q_input]})[0]
        ...
        if t % LOG_EVERY_N_STEPS == 0 and model_initialized:
            ...
            logz.log_tabular("mean_rwd", mean_episode_reward)
            logz.log_tabular("best_mean_rwd", best_mean_episode_reward)
